 ...
 * @file    bsp_xxx.c
 * @author  fire
 * @attention Modefied for the project.
 * @version V1.0
 * @date    2013-xx-xx
...
uint8_t SPI_NRF_RW(uint8_t dat) {
  uint8_t ret;
  HAL_SPI_Transmit(&NRF_SPI, &dat, sizeof(dat), NRF_SPI_TIMEOUT);
  HAL_SPI_Receive(&NRF_SPI, &ret, sizeof(ret), NRF_SPI_TIMEOUT);
  return ret;
}
...
void NRF_TX_Mode(void) {
  ...
  /*CE拉高，进入发送模式*/
  NRF_CE_HIGH();
  SysTimer_Delay_us(20);  // CE要拉高一段时间才进入发送模式
}
